...
navs=[('index.html','首頁'),('about.html','關於我們'),('services.html','服務項目'),('works.html','案例分享'),('shop.html','周邊商品'),('contact.html','聯絡我們')]
CART_ICON='<svg viewBox="0 0 24 24" fill="none"><path d="M4 5h2l1.6 10.6a2 2 0 0 0 2 1.7h7.6a2 2 0 0 0 2-1.6L20.5 8.5H7" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><circle cx="10" cy="20" r="1.4" fill="currentColor"/><circle cx="17" cy="20" r="1.4" fill="currentColor"/></svg>'
def page(name,title,body,prefix='',chrome=True):
 ...
 cart_link=f'<a class="cart-link" href="{prefix}cart.html" aria-label="購物車">{CART_ICON}<span id="cart-count" class="cart-badge" hidden>0</span></a>'
 chrome_html=f'''<a class="skip" href="#main">跳至主要內容</a><header><a class="brand" href="{prefix}welcome.html" aria-label="CTDC 臺灣"><div><img class="logo-mark" src="{prefix}assets/{BRAND_LOGO}" alt="CTDC"></div><div class="brand-name">美淑琳<br>設計顧問有限公司</div></a><button class="menu" aria-controls="nav" aria-expanded="false">選單 ☰</button><nav id="nav" aria-label="主要導覽">{nav}</nav>{cart_link}</header><main id="main">{body}</main><footer><div class="footer-top"><div><a class="brand" href="{prefix}welcome.html"><div><img class="logo-mark" src="{prefix}assets/{BRAND_LOGO}" alt="CTDC"></div></a><p class="footer-info" style="margin-top:22px">美淑琳設計顧問有限公司<br>統一編號 60677831</p></div><div class="footer-nav">{nav}</div></div><div class="footer-bottom"><span>© 2026 CTDC Taiwan. All rights reserved.</span><span>空間有形，生活無限。</span></div></footer><a class="backtop" href="#top" aria-label="回到頁首">↑</a>''' if chrome else f'<main id="main">{body}</main>'
...
page('shop.html','周邊商品',heading('周邊商品','SHOP')+'<section class="wrap shop-layout" style="padding-bottom:140px"><aside class="shop-cats" id="shop-cats" aria-label="商品分類"></aside><div><div id="shop-status"></div><div id="shop-grid" class="project-grid" aria-live="polite"><p class="empty">載入中…</p></div></div></section>')
page('product.html','商品詳情','<div class="wrap breadcrumb"><a href="shop.html">周邊商品</a> / <span id="product-crumb">商品詳情</span></div><section class="wrap shop-detail" id="product-detail" style="padding-bottom:160px" aria-live="polite"><p class="empty">載入中…</p></section>')
page('cart.html','購物車',heading('購物車','CART')+'<section class="wrap shop-checkout" style="padding-bottom:140px"><div class="receipt-card"><div id="cart-list" aria-live="polite"><p class="empty">載入中…</p></div><div id="cart-summary" class="cart-total" hidden><span>總金額</span><strong id="cart-total-amount">NT$0</strong></div></div><a class="text-link" id="checkout-link" href="checkout.html" style="margin-top:30px" hidden>前往結帳</a></section>')
page('checkout.html','結帳',heading('結帳','CHECKOUT')+'<section class="wrap shop-checkout" style="padding-bottom:140px"><div class="receipt-card"><div id="checkout-summary" aria-live="polite"><p class="empty">載入中…</p></div></div><form id="checkout-form" class="contact-form" hidden><div class="form-grid"><label class="form-field"><span>收件人姓名 *</span><input type="text" name="name" required></label><label class="form-field"><span>聯絡電話 *</span><input type="tel" name="phone" required></label><label class="form-field"><span>電子信箱 *</span><input type="email" name="email" required></label><label class="form-field"><span>收件地址 *</span><input type="text" name="address" required></label></div><label class="form-field" style="margin-bottom:26px"><span>備註</span><textarea name="note" rows="3"></textarea></label><div class="form-actions"><button type="submit">前往付款</button><p class="form-status" id="checkout-status" role="status"></p></div></form></section>')
page('order-status.html','訂單狀態',heading('訂單狀態','ORDER STATUS')+'<section class="wrap" style="padding-bottom:100px;max-width:720px"><div id="order-status-box" aria-live="polite"><p class="empty">查詢中…</p></div></section>')
