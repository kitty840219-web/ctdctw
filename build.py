...
def recommend_track(items):
 track_items=items+items
 cards_html=''.join(f'<a class="recommend-card" href="projects/{c["id"]}.html"><div class="image">{photo(c["photos"][0],c["title"]+"案例照片")}</div><h3>{c["title"]}</h3></a>' for c in track_items)
...
for c in cases:
 ...
 body=f'<div class="wrap"><div class="breadcrumb"><a href="../works.html">案例分享</a> / {cats[c["category"]]}</div><div class="detail-title"><p class="eyebrow">CTDC PROJECT / {cats[c["category"]]}</p><h1>{c["title"]}</h1></div><div class="gallery">'+''.join(photo(p,c['title']+f'・空間照片 {i+1}',eager=i==0,prefix='../') for i,p in enumerate(c['photos']))+f'</div><div class="detail-info">{info}</div><a class="text-link" style="margin:25px 0 80px" href="../works.html">返回案例分享</a></div>'
