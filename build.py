...
def page(name,title,body,prefix=''):
 ...
 txt=f'''<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}｜CTDC 臺灣・美淑琳設計顧問</title><meta name="description" content="美淑琳設計顧問有限公司，CTDC 臺灣。整合設計、工程與專案管理，讓空間的每一個細節，回應生活。"><meta name="theme-color" content="#f4f3ef"><link rel="stylesheet" href="{prefix}style.css"><link rel="icon" href="{prefix}favicon.svg" type="image/svg+xml"><script src="{prefix}site.js" defer></script></head><body id="top"><a class="skip" href="#main">跳至主要內容</a><header><a class="brand" href="{prefix}index.html" aria-label="CTDC 臺灣首頁"><div><img class="logo-mark" src="{prefix}assets/{BRAND_LOGO}" alt="CTDC ・ 美淑琳設計顧問有限公司"></div></a><button class="menu" aria-controls="nav" aria-expanded="false">選單 ☰</button><nav id="nav" aria-label="主要導覽">{nav}</nav></header><main id="main">{body}</main><footer><div class="footer-top"><div><a class="brand" href="{prefix}index.html"><div><img class="logo-mark" src="{prefix}assets/{BRAND_LOGO}" alt="CTDC ・ 美淑琳設計顧問有限公司"></div></a><p class="footer-info" style="margin-top:22px">美淑琳設計顧問有限公司<br>統一編號 60677831</p></div><div class="footer-nav">{nav}</div></div><div class="footer-bottom"><span>© 2026 CTDC Taiwan. All rights reserved.</span><span>空間有形，生活無限。</span></div></footer><a class="backtop" href="#top" aria-label="回到頁首">↑</a></body></html>'''
