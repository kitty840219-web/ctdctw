...
for c in cases:
 ...
 body=f'<div class="wrap"><div class="breadcrumb"><a href="../works.html">案例分享</a> / {cats[c["category"]]}</div><div class="detail-title"><p class="eyebrow">CTDC PROJECT / {cats[c["category"]]}</p><h1>{c["title"]}</h1></div><div class="gallery">'+''.join(photo(p,c['title']+f'・空間照片 {i+1}',eager=i==0,prefix='../') for i,p in enumerate(c['photos']))+f'</div><div class="detail-info">{info}</div><a class="text-link" style="margin:25px 0 60px" href="../works.html">返回案例分享</a></div>'+recommend_track(related_cases(c,cases),prefix='../')
